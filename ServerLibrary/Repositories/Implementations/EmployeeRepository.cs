...
namespace ServerLibrary.Repositories.Implementations
{
    public class EmployeeRepository(AppDbContext appDbContext) : IGenericRepositoryInterface<Employee>
    {
        ...
        {
            if (!await CheckName(item.Name!)) return new GeneralResponse(false, "Personel zaten eklenmiş.");
        ...
        public async Task<GeneralResponse> Update(Employee employee)
        {
            ...
            if (findUser is null) return new GeneralResponse(false, "Personel bulunamadı.");
        ...
        private static GeneralResponse NotFound() => new(false, "Personel bulunamadı.");
